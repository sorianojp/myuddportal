...
class GradeApiController extends Controller
{
    ...
    {
        $userIndex = $request->input('USER_INDEX'); // from Flutter request
        ...
        $gradeOrder = ['Final' => 1, 'Midterm' => 2, 'Prelim' => 3];

        $allGrades = collect($finalGrades)
            ->merge($termGrades)
            ->sortBy(function ($grade) use ($gradeOrder) {
                return [
                    -$grade->SY_FROM,
                    -(int) $grade->SEMESTER,
                    $gradeOrder[$grade->GRADE_NAME] ?? 99,
                    $grade->SUB_CODE,
                ];
            })
            ->groupBy(fn($grade) =>
        ...
        return response()->json([
            'grades' => $allGrades->map(fn($group) => collect($group)->map(fn($g) => (array) $g)->values()),
        ]);
    }
}
